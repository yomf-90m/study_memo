...
```
[例] p要素がブロックボックス、a要素がインラインボックス
[HTML]
<p>こんにちは。こんにちは。こんにちは。</p>
<p>こんにちは。<a href="">リンク</a>
<a href="">リンク</a>こんにちは。</p>
<p>こんにちは。こんにちは。こんにちは。</p>
...
上下方向に関しては他の要素は影響しないので押しのけられずに、他の要素はそのままの位置に留まる。

```
[例] a要素(インラインブロック)にborderを指定
a {
  background: skyblue;
  border: 8px solid blue;
}
...
→インラインボックスだけれども、サイズを指定することができる、という仕様になっている。

display: inline-block; と同じ挙動になる。
